...
Node* BST::deleteNode(Node *currNode, int value){
    ...
    // We found the node with the value
        ...
        ///Replace with Minimum from right subtree
            toDelete = getMinValueNode(toDelete->right);
            currNode->key = toDelete->key;
            deleteNode(currNode->right, toDelete->key);
        }
...
bool isValidHelper(Node* root){
    if (root == NULL){
        return true;
    }

    if (!isValidHelper(root->left)){
        return false;
    } 
    if (!isValidHelper(root->right)){
        return false;
    }
